...
public class MaxCardinalityRestrictionConditionChecker extends AbstractConditionChecker {
	...
	public MaxCardinalityRestrictionConditionChecker(OntologyABoxHelper ah,
	...
	public boolean checkCondition(OWLObjectPropertyExpression role,
		...
		for (OWLObjectPropertyExpression r : involvedRoles) {
			...
			int minCard = this.minCardinalities.get(r) + 1;
		...
		// 3. obj classes are subsumed by every Filler Concept.
		// 4. sub does not have enough neighbors
		return !container.isSubclassOf(subClasses, involvedLHSClasses)
				&& !container.isDisjointFrom(subClasses, involvedLHSClasses)
